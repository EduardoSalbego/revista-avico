...
<body class="antialiased" style="height: 100vh;">
    ...
    <nav class="navbar navbar-dark bg-dark">
        ...
        <div class="hidden fixed top-0 right-0 px-6 py-3 sm:block">
            @auth
            <a href="{{ url('/home') }}" id="home" class="text-sm text-gray-700 dark:text-gray-500 underline">Home</i></a>
            @else
            <a href="{{ route('login') }}" id="Btnlogin" class="text-sm text-gray-700 dark:text-gray-500 underline">Log in</a>

            @if (Route::has('register'))
            <a href="{{ route('register') }}" id="Btnregistro" class="ml-4 text-sm text-gray-700 dark:text-gray-500 underline">Register</a>
            @endif
            @endauth
        </div>
        @endif
