            ...
            <div class="d-flex flex-column gap-3">
                @foreach ($submissoes as $submissao)
                    @php
                        ...
                        $bloqueados = $pareceres
                            ->filter(fn($p) => !is_null($p->decisao))
                            ->pluck('revisor_id')
                            ->toArray();
                        $recusas = $submissao->revisoresAtribuidos->where('pivot.status', 'recusado');
                    ...
                    <div class="card p-4 mb-2 shadow-sm border-0">
                        ...
                        {{-- Cabecalho --}}
                        <div
                            class="d-flex justify-content-between align-items-start flex-wrap gap-2 mb-3 border-bottom pb-3">
                            <div>
                        ...
                        @if (!$submissao->isRejeitado())
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="d-flex flex-wrap gap-2">
                                        @foreach ($submissao->revisoresAtribuidos as $r)
                                            {{-- PULA O REVISOR SE ELE JÁ FOI SUBSTITUÍDO (Limpa a interface do Editor) --}}
                                            @if ($r->pivot->status === 'substituido')
                                                @continue
                                            @endif
                                            @php
                                                $p = $pareceres->firstWhere('revisor_id', $r->id);
                                                $statusPiv = $r->pivot->status; // Pega o status exato da tabela pivot
                                                $jaRevisou = $p && !is_null($p->decisao);
                                                $recusou = $statusPiv === 'recusado';
                                                $aguardando = $statusPiv === 'pendente';
                            ...
                            @if ($recusas->isNotEmpty() && $submissao->status === 'em_revisao')
                                <div
                                    class="alert alert-danger d-flex align-items-start gap-3 mt-2 mb-3 shadow-sm border-0">
                                    <i class="fas fa-user-times fs-4 text-danger flex-shrink-0 mt-1"></i>
                                    <div class="w-100">
                                        <strong>
                                            {{ $recusas->count() === 1 ? '1 revisor recusou' : $recusas->count() . ' revisores recusaram' }}
                                            a tarefa.
                                        </strong>
                                        <p class="mb-2 small mt-1">
                                            Clique em <strong>"Substituir"</strong> ao lado do nome para designar um
                                            substituto.
                                        </p>

                                        {{-- ── Lista de Sugestões de Substitutos ── --}}
                                        @foreach ($recusas as $recusa)
                                            @php
                                                // Busca o parecer correspondente a este revisor
                                                $parecerRecusa = $pareceres->firstWhere('revisor_id', $recusa->id);
                                            @endphp

                                            @if ($parecerRecusa && $parecerRecusa->substituto_sugerido_nome)
                                                <div
                                                    class="bg-white bg-opacity-75 p-2 rounded border border-danger border-opacity-25 mt-2 small">
                                                    <i class="fas fa-lightbulb text-warning me-1"></i>
                                                    <strong>{{ $recusa->user->name ?? $recusa->name }}</strong> sugeriu
                                                    para o seu lugar
                                                    <span
                                                        class="text-dark fw-semibold">{{ $parecerRecusa->substituto_sugerido_nome }}</span>
                                                    @if ($parecerRecusa->substituto_sugerido_email)
                                                        <a href="mailto:{{ $parecerRecusa->substituto_sugerido_email }}"
                                                            class="text-decoration-none text-primary ms-1">
                                                            ({{ $parecerRecusa->substituto_sugerido_email }})
                                                        </a>
                                                    @endif
                                                </div>
                                            @endif
                                ...
                                <div class="card p-3 mt-2 bg-light border-0">
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-4 border-start">
                                            <label class="form-label fw-bold small text-danger">Rejeitar na
                                                Triagem</label>
                                            <form action="{{ route('editor.submissoes.decidir', $submissao->id) }}"
                                                method="POST"
                                                onsubmit="return confirm('Rejeitar esta submissao direto da triagem?')">
                                                @csrf
                                                @method('PATCH')
                                                <input type="hidden" name="status" value="rejeitado">
                                                <textarea name="observacoes" class="form-control form-control-sm mb-3 shadow-sm" rows="5"
                                                    placeholder="Motivo da rejeicao (obrigatorio)..." required></textarea>
                            ...
                            @if ($submissao->status === 'em_revisao')
                                ...
                                @if (!$todosResponderam && $recusas->isEmpty())
                                    <div
                                        class="alert alert-info py-3 mt-3 mb-0 shadow-sm border-0 d-flex align-items-center">
                                        ...
                                        <div class="p-3 bg-light rounded border">
                                            ...
                                            <form action="{{ route('editor.submissoes.decidir', $submissao->id) }}"
                                                ...
                                                <div class="row g-3">
                                                    ...
                                                    <div class="col-md-4">
                                                        <label
                                                            class="form-label small fw-bold text-muted mb-1">Veredito</label>
                                                    ...
                                                    <div class="col-md-8 d-flex align-items-end justify-content-end">
                                                        <button type="submit" class="btn btn-primary px-5 shadow-sm">
                                                            <i class="fas fa-check-circle me-1"></i> Concluir e
                                                            Notificar Autor
                                                        </button>
                                                    </div>
                                                </div>
                ...
                <form id="form-substituicao" action="" method="POST">
                    ...
                    <div class="modal-body pt-3">
                        ...
                        <div>
                            ...
                            <div id="modal-sem-substitutos" class="alert alert-warning small py-2 mt-2"
                                style="display:none;">
    ...
    <script>
        ...
        function _renderEquipe(sid) {
            ...
            Object.entries(equipe).forEach(([id, nome]) => {
                ...
                div.className =
                    'd-flex justify-content-between align-items-center bg-light border rounded px-2 py-1';
                div.innerHTML = `
        ...
        window.abrirModalSubstituicao = function(submissaoId, substituirId, nomeAntigo) {
            ...
            const estado = estadoSubmissoes[submissaoId] ?? {
                atribuidos: [],
                recusados: []
            };
